...
from ..models import QuestionFree #부모 디렉터리에 존재하므로 ..붙여주기
...
def index(request): #request는 장고 프레임워크에 의해 자동으로 전달되는 HTTP 요청 객체
    ...
    # 정렬
    if so == 'recommend':
        question_list = QuestionFree.objects.annotate(num_voter=Count('voter_free')).order_by('-num_voter_free', '-create_date_free') #추천수는 장고의 annotate를 이용하여 Count함수를 통해 구할수있음.
    elif so == 'popular':
        question_list = QuestionFree.objects.annotate(num_answer=Count('answer_free')).order_by('-num_answer_free', '-create_date_free')
    else:  # recent
        question_list = QuestionFree.objects.order_by('-create_date_free')
    ...
    return render(request, 'free/question_list.html', context) #템플릿파일

def detail(request, question_id): #전달되는 매개변수가 request외에 question_id가 하나 더 추가
    """
    free 내용 출력
    """
    question = get_object_or_404(QuestionFree, pk=question_id)#pk는 QuestionFree모델의 기본키(Primary Key)인 id를 의미
