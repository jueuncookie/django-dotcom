...
from ..models import QuestionDouble #부모 디렉터리에 존재하므로 ..붙여주기
...
def index(request): #request는 장고 프레임워크에 의해 자동으로 전달되는 HTTP 요청 객체
    ...
    # 정렬
    if so == 'recommend':
        question_list = QuestionDouble.objects.annotate(num_voter=Count('voter_double')).order_by('-num_voter_double', '-create_date_double') #추천수는 장고의 annotate를 이용하여 Count함수를 통해 구할수있음.
    elif so == 'popular':
        question_list = QuestionDouble.objects.annotate(num_answer=Count('answer_double')).order_by('-num_answer_double', '-create_date_double')
    else:  # recent
        question_list = QuestionDouble.objects.order_by('-create_date_double')
    ...
    return render(request, 'doubleboard/question_list.html', context) #템플릿파일

def detail(request, question_id): #전달되는 매개변수가 request외에 question_id가 하나 더 추가
    """
    doubleboard 내용 출력
    """
    question = get_object_or_404(QuestionDouble, pk=question_id)#pk는 QuestionDouble모델의 기본키(Primary Key)인 id를 의미
